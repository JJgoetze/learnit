...
#define LOG(LEVEL)            \
    LogMessage(::LOGLEVEL_##LEVEL, __FILE__, __LINE__,false)
#define SYSLOG(LEVEL)            \
	LogMessage(::LOGLEVEL_##LEVEL, __FILE__, __LINE__,true)
...
class LogMessage {
public:
    LogMessage(LogLevel level ,const char* filename, int line,bool ifSysLog);
    ~LogMessage();
