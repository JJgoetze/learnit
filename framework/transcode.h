...
#define FACTOR 2
#define PACKETLEN sizeof(uint16_t)
...
protected:
	BinaryData* Encode(const BinaryData* message);
	BinaryData* Decode(const BinaryData* message);

	bool IsDataEnough(BinaryData* message, size_t* actualSize);

	void Write(BinaryData* &dest, const uint8_t* src, size_t len);
	void Read(uint8_t* dest, const BinaryData* src, size_t &offset, size_t len);

	virtual BinaryData* doEncode(const BinaryData* message) = 0;
