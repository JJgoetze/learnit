...
void Transcode::Write(BinaryData* &dest, const uint8_t* src, size_t len) {
...
bool Transcode::IsDataEnough(BinaryData* message, size_t* actualSize) {
	if (message == nullptr) 
	{
		return false;
	} 
	...
	{
		LOG(WARNING) << " TcpConnection , Buffer " << message << ",  Receive Data is not Enough." << " Total:" << *actualSize << " Actual:" << message->offset;
		return false;
	} 
	else 
	{
...
BinaryData* Transcode::Encode(const BinaryData* message) {
	if (message == nullptr)	{
		LOG(INFO) << "Encode message is null";
		return nullptr;
	}
	if (kernel_ != nullptr) {
...
BinaryData* Transcode::Decode(const BinaryData* message) {
	if (message == nullptr)	{
		LOG(INFO) << "Decode message is null";
		return nullptr;
	}
	if (kernel_ != nullptr) {
