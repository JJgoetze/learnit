...
#include "framework/scope_guard.h"

#include "framework/tcp_server_message_codes.h"
...
TcpConnection::TcpConnection(Async* async_owner, Server* server) : async_owner_(async_owner)
    , server_(server)
    , remote_port_(0)
    , running_(false)
    , reading_(false)
...
std::string TcpConnection::GetSessionId()  
{
	if (session_id.empty())
	{
		session_index_++; 
		char session[10] = { '\0' };
		snprintf(session, sizeof(session), "%lld", session_index_);
...
void TcpConnection::StartRead() {
    ...
    if (err == 0) {
		LOG(WARNING) << this << " TcpConnection::OnMessage, uv_read_start  ";
        reading_ = true;
    } else {
...
void TcpConnection::Write(EndpointTranscode &transcoder) {
...
void TcpConnection::Write(const uint8_t *data, size_t len) {
    ...
    int ret = uv_write(req, reinterpret_cast<uv_stream_t*>(&stream_.tcp), &buf, 1, &TcpConnectionCallback::do_write);
...
size_t TcpConnection::Read(EndpointTranscode& transcoder)
{
	...
	if (isEnough)
	{
		...
		//TODO,actualLength 小于4字节的包:连续收到这样的包应该断开该连接 ！！！
