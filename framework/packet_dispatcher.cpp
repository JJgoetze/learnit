...
// 	static int32_t time_map_num;
    ...
    for (auto it = active_connections->begin(); it != active_connections->end(); ++it) {
        ...
        while (count < PROCESS_PACKET_COUNT_PER_LOOP && ((*it)->Read(packet)) > 0) {
        	//dispatch packet
        	uint32_t opcode = packet.GetOpcode();
			if (opcode == 3)
			{
				//心跳包
				(*it)->last_keep_alive_time = time(nullptr); 

				//PacketTranscode keep_alive_packet_resp; 
				//keep_alive_packet_resp.SetOpcode(4); 
				//(*it)->Write(keep_alive_packet_resp);

				continue; 
			}

			//int64_t t_begin = CbUtility::get_start_time();

        	if (opcode != 3 && opcode != 105) 
			{
        		//LOG(INFO) << "Dispatcher Get Packet, Opcode is " << opcode;
        	}
        	auto iter = dispatchRules_.find(opcode);
        	if (iter != dispatchRules_.end()) {
				LOG(DEBUG) << "Dispatcher Get Packet, Opcode is " << opcode << " TcpConnection " << (*it);
        		GameService *service = iter->second;
        		connToGameService_[*it].insert(service);
        		service->OnRecvPacket(*it, packet);
				if (!packet.IsGood())
				{
					LOG(ERROR) << " TcpConnection " << (*it) << " OnRecvPacket Not Good "; 
				}
        	} else {
